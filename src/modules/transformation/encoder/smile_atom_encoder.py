...
class SmileAtomEncoder(VerboseTransformationBlock):
    ...
    def custom_transform(self, x: XData) -> XData:
        ...
        if self.convert_molecules:
            smiles_enc = joblib.Parallel(n_jobs=-1)(joblib.delayed(encode_molecule)(smile, MAX_ENC_SIZE_MOLECULE) for smile in tqdm(smiles, desc="Encoding Molcules Atomise"))
            x.molecule_ecfp = np.stack(smiles_enc)

        if self.convert_building_blocks:
            bb1_encoding = joblib.Parallel(n_jobs=-1)(
                joblib.delayed(encode_block_1)(smile, MAX_ENC_SIZE_BB1) for smile in tqdm(x.bb1_smiles, desc="Encoding block 1 Atomwise")
            )
...
def encode_molecule(smile: str, padding: int) -> npt.NDArray[np.uint8]:
...
def encode_block(block: str, padding: int) -> npt.NDArray[np.uint8]:
...
def encode_block_1(block: str, padding: int) -> npt.NDArray[np.uint8]:
    """Encode an atom based on enc dictionary."""
    return encode_block(remove_substructures(block), padding)
