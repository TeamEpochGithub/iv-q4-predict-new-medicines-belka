...
class SegmentMolecule(VerboseTransformationBlock):
    ...
    def parallel_segment(self, smiles: list[str],padding_size: int) -> npt.NDArray[np.str_]:
        ...
        with ProcessPoolExecutor() as executor:
            futures = [executor.submit(self.segment_molecule, smiles=chunk, window_size=self.window_size,
                                       padding_size= padding_size) for chunk in chunks]
    ...
    def custom_transform(self, data: XData) -> XData:
        ...
        if data.bb1_smiles is None and data.bb2_smiles is None and data.bb3_smiles is None:
